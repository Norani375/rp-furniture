...
export default function Manufacturing() {
  ...
  const printManufacturing = () => printMinimalDocument({
    title: 'فاکتور / گزارش تولید',
    subtitle: selected?.productName || 'BOM',
    party: 'واحد تولید',
    headers: ['ماده', 'واحد', 'مقدار', 'قیمت', 'قیمت کل'],
    rows: selected?.materials.map((m) => [m.name, m.unit, m.quantity, AFN(m.unitCost), AFN(m.quantity * m.unitCost)]) || [],
    totals: [
      { label: 'هزینه مواد', value: AFN(selected?.materials.reduce((s, m) => s + m.quantity * m.unitCost, 0) || 0) },
      { label: 'هزینه یک واحد', value: AFN(cost) },
    ],
  });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2"><button onClick={printManufacturing} className="rounded-xl border border-slate-300 px-4 py-2 text-sm text-slate-600 hover:bg-slate-50">پرینت</button><div className="rounded-full bg-indigo-50 px-3 py-1 text-xs font-medium text-indigo-700">BOM فعال: {recipes.length}</div></div>
      </div>
        ...
        <div className="space-y-2">
          ...
          {orders.slice().reverse().map((o) => (
            <div key={o.id} className="flex items-center justify-between rounded-xl border border-slate-100 px-4 py-3">
              ...
              <div className="flex items-center gap-3"><div className="text-left"><p className="text-sm font-bold text-slate-900">{AFN(o.totalCost)}</p><p className="text-xs text-emerald-600">{o.quantity} دانه تکمیل شد</p></div><RecordActions compact onDelete={() => deleteOrder(o.id)} onPrint={printManufacturing} /></div>
            </div>
          ))}
        </div>
      </div>
