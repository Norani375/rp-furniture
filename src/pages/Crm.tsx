  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div><h2 className="text-xl font-bold text-slate-900">CRM — مدیریت ارتباط با مشتریان</h2><p className="text-sm text-slate-500">{customers.length} مشتری · {interactions.length} تعامل</p></div>
        <button onClick={() => setShowForm(true)} className="flex items-center gap-2 rounded-xl bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-700"><Plus size={16} /> تعامل جدید</button>
      </div>
      ...
      <div className="flex gap-2 border-b border-slate-200">
        <button onClick={() => setTab('customers')} className={`px-4 py-2 text-sm font-medium border-b-2 ${tab === 'customers' ? 'border-indigo-600 text-indigo-700' : 'border-transparent text-slate-500'}`}>مشتریان ({customers.length})</button>
          ...
          {customers.map((c) => (
            <div key={c.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
              <div className="flex items-start justify-between">
                ...
                <span className={`rounded-full px-2.5 py-1 text-xs font-medium ${c.status === 'active' ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-700'}`}>{c.status === 'active' ? 'فعال' : 'غیرفعال'}</span>
              </div>
