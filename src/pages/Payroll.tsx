...
export default function Payroll() {
  ...
  const printPayroll = () => printMinimalDocument({
    title: 'فاکتور / گزارش حقوق و دستمزد',
    subtitle: 'کارکنان',
    party: 'منابع انسانی',
    headers: ['کارمند', 'دپارتمان', 'سمت', 'حقوق', 'وضعیت'],
    rows: employeeList.map((e) => [`${e.firstName} ${e.lastName}`, e.department, e.position, AFN(e.salary), e.status === 'active' ? 'فعال' : 'غیرفعال']),
    totals: [{ label: 'جمع حقوق ماهانه', value: AFN(employeeList.reduce((s, e) => s + e.salary, 0)) }],
  });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2"><button onClick={printPayroll} className="rounded-xl border border-slate-300 px-4 py-2 text-sm text-slate-600 hover:bg-slate-50">پرینت</button><button onClick={() => setShowForm(true)} className="flex items-center gap-2 rounded-xl bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-700"><Plus size={16} /> ثبت حقوق</button></div>
      </div>
