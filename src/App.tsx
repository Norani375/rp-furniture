...
const roleAccess: Record<UserRole, string[]> = {
  ...
  accountant: ['dashboard', 'accounting', 'banking', 'currencies', 'installments', 'tax', 'reports', 'notifications', 'settings', 'audit', 'backup'],
...
async function testBackendConnection(): Promise<'online' | 'offline'> {
  try {
...
function AppInner() {
  ...
  useEffect(() => {
    testBackendConnection().then((status) => setDbStatus(status));
      ...
      <main className="flex-1 overflow-y-auto">
        ...
        <div className="sticky top-0 z-40 flex items-center justify-between border-b border-slate-200 bg-white/90 px-6 py-3 backdrop-blur print:hidden">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <Bell size={14} />
              {notificationCount > 0 && (
                <span className="absolute -left-1 -top-1 rounded-full bg-red-600 px-1.5 py-0.5 text-[10px] text-white">
                  {notificationCount}
                </span>
              )}
            </button>
            ...
            >
              ...
              <span
                className={`h-2 w-2 rounded-full ${
                  dbStatus === 'online' ? 'bg-emerald-500' : dbStatus === 'offline' ? 'bg-amber-500' : 'bg-slate-400 animate-pulse'
                }`}
              />
              {dbStatus === 'online' ? 'Backend متصل' : dbStatus === 'offline' ? 'حالت محلی' : 'در حال بررسی...'}
            </button>
          </div>
        </div>
        ...
        <footer className="mt-10 border-t border-slate-200 bg-white py-4 print:hidden">
          <div className="mx-auto max-w-7xl px-4 text-center text-xs text-slate-500">
            سیستم مدیریت کسب‌وکار — نسخه ۱.۰
          </div>
        </footer>
      </main>
    </div>
